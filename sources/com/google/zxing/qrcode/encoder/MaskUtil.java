...
final class MaskUtil {
    ...
    static int applyMaskPenaltyRule2(ByteMatrix byteMatrix) {
        ...
        byteMatrix = byteMatrix.getHeight();
        int i = 0;
        int i2 = 0;
        while (i < byteMatrix - 1) {
            int i3 = i2;
            i2 = 0;
            while (i2 < width - 1) {
                byte b = array[i][i2];
    ...
    static int applyMaskPenaltyRule3(ByteMatrix byteMatrix) {
        ...
        while (i < byteMatrix) {
            ...
            while (i2 < width) {
                ...
                int i5 = i + 6;
                if (i5 < byteMatrix && array[i][i2] == (byte) 1 && array[i + 1][i2] == (byte) 0 && array[i + 2][i2] == (byte) 1 && array[i + 3][i2] == (byte) 1 && array[i + 4][i2] == (byte) 1 && array[i + 5][i2] == (byte) 0 && array[i5][i2] == (byte) 1 && (isWhiteVertical(array, i2, i - 4, i) || isWhiteVertical(array, i2, i + 7, i + 11))) {
                    i3++;
                }
                i2++;
            }
    ...
    static boolean getDataMaskBit(int i, int i2, int i3) {
        switch (i) {
            ...
            default:
                ...
                i3.append("Invalid mask pattern: ");
                i3.append(i);
    ...
    private static int applyMaskPenaltyRule1Internal(ByteMatrix byteMatrix, boolean z) {
        ...
        while (i < height) {
            ...
            byte b = (byte) -1;
            while (i2 < width) {
                byte b2 = z ? byteMatrix[i][i2] : byteMatrix[i2][i];
                if (b2 == b) {
                    i4++;
                } else {
                    if (i4 >= 5) {
