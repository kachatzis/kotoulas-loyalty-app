...
final class ColorCutQuantizer {
    ...
    private static final Comparator<Vbox> VBOX_COMPARATOR_VOLUME = new C02931();
    final int[] mColors;
    ...
    static class C02931 implements Comparator<Vbox> {
        C02931() {
        }
    ...
    ColorCutQuantizer(int[] iArr, int i, Filter[] filterArr) {
        ...
        filterArr = new int[32768];
        this.mHistogram = filterArr;
        int i3 = 0;
        for (i2 = 0; i2 < iArr.length; i2++) {
            int quantizeFromRgb888 = quantizeFromRgb888(iArr[i2]);
            iArr[i2] = quantizeFromRgb888;
            filterArr[quantizeFromRgb888] = filterArr[quantizeFromRgb888] + 1;
        }
        int i4 = null;
        i2 = 0;
        while (i4 < filterArr.length) {
            if (filterArr[i4] > null && shouldIgnoreColor(i4)) {
                filterArr[i4] = null;
            }
        ...
        this.mColors = iArr;
        int i5 = 0;
        for (quantizeFromRgb888 = 0; quantizeFromRgb888 < filterArr.length; quantizeFromRgb888++) {
            if (filterArr[quantizeFromRgb888] > null) {
                int i6 = i5 + 1;
        ...
        if (i2 <= i) {
            ...
            while (i3 < i) {
                i2 = iArr[i3];
                this.mQuantizedColors.add(new Swatch(approximateToRgb888(i2), filterArr[i2]));
                i3++;
            }
            return;
        }
