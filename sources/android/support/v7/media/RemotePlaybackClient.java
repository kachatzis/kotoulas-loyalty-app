...
public class RemotePlaybackClient {
    ...
    private final class ActionReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (stringExtra != null) {
                if (stringExtra.equals(RemotePlaybackClient.this.mSessionId) != null) {
                    ...
                    if (context.equals(ACTION_ITEM_STATUS_CHANGED)) {
                        ...
                        if (RemotePlaybackClient.DEBUG != null) {
                            context = RemotePlaybackClient.TAG;
                    ...
                    } else if (context.equals(ACTION_SESSION_STATUS_CHANGED)) {
                        if (fromBundle == null) {
                    ...
                    } else if (context.equals(ACTION_MESSAGE_RECEIVED) != null) {
                        if (RemotePlaybackClient.DEBUG != null) {
                            context = RemotePlaybackClient.TAG;
                            stringBuilder = new StringBuilder();
            ...
            intent.append("Discarding spurious status callback with missing or invalid session id: sessionId=");
            intent.append(stringExtra);
            Log.w(context, intent.toString());
        }
    }
    ...
    public RemotePlaybackClient(Context context, RouteInfo routeInfo) {
        if (context == null) {
            ...
            routeInfo.addAction(ActionReceiver.ACTION_ITEM_STATUS_CHANGED);
            routeInfo.addAction(ActionReceiver.ACTION_SESSION_STATUS_CHANGED);
            routeInfo.addAction(ActionReceiver.ACTION_MESSAGE_RECEIVED);
            this.mActionReceiver = new ActionReceiver();
            context.registerReceiver(this.mActionReceiver, routeInfo);
            routeInfo = new Intent(ActionReceiver.ACTION_ITEM_STATUS_CHANGED);
            routeInfo.setPackage(context.getPackageName());
            this.mItemStatusPendingIntent = PendingIntent.getBroadcast(context, 0, routeInfo, 0);
            routeInfo = new Intent(ActionReceiver.ACTION_SESSION_STATUS_CHANGED);
            routeInfo.setPackage(context.getPackageName());
            this.mSessionStatusPendingIntent = PendingIntent.getBroadcast(context, 0, routeInfo, 0);
            routeInfo = new Intent(ActionReceiver.ACTION_MESSAGE_RECEIVED);
            routeInfo.setPackage(context.getPackageName());
            this.mMessagePendingIntent = PendingIntent.getBroadcast(context, 0, routeInfo, 0);
            detectFeatures();
        } else {
                ...
                if (bundle != null) {
                    ...
                    if (inferMissingResult != null) {
                        ...
                        try {
                            ...
                            if (!(intent.getAction().equals(MediaControlIntent.ACTION_END_SESSION) == null || inferMissingResult.equals(RemotePlaybackClient.this.mSessionId) == null)) {
